...
public class PC_EnergyGrid extends PC_Grid<PC_EnergyGrid, PC_IEnergyGridTile, PC_EnergyNode<?>, PC_EnergyEdge> {
	...
	public void doEnergyTick(){
		...
		if(energy>0 && !buffer.isEmpty()){
			...
			Arrays.sort(array);
			float startLevel = array[0].level;
			int nums = 1;
	...
	protected PC_EnergyNode<?> createNode(PC_IEnergyGridTile tile) {
		if(tile instanceof PC_IEnerigyGridConduit){
			return new PC_EnergyNodeConduit(this, (PC_IEnerigyGridConduit)tile);
		}else if(tile instanceof PC_IEnerigyGridBuffer){
			return new PC_EnergyNodeBuffer(this, (PC_IEnerigyGridBuffer)tile);
		}else if(tile instanceof PC_IEnerigyGridProvider){
			return new PC_EnergyNodeProvider(this, (PC_IEnerigyGridProvider)tile);
		}else if(tile instanceof PC_IEnerigyGridConsumer){
			return new PC_EnergyNodeConsumer(this, (PC_IEnerigyGridConsumer)tile);
		}
