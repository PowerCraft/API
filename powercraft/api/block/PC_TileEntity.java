...
public class PC_TileEntity extends TileEntity {
	...
	public boolean renderWorldBlock(int modelId, RenderBlocks renderer) {
		return PC_Renderer.renderBlockInWorld(this.worldObj, this.xCoord, this.yCoord, this.zCoord, getBlockType(),
				modelId, renderer);
	}
	...
	public final void onClientMessageCheck(EntityPlayer player, NBTTagCompound nbtTagCompound, long session,
		...
		if (pSession != null && pSession.dimension == PC_Utils.getDimensionID(this.worldObj)
				&& pSession.x == this.xCoord && pSession.y == this.yCoord && pSession.z == this.zCoord
	...
	public void sendMessage(NBTTagCompound nbtTagCompound) {
		if (isClient()) {
			...
			PC_PacketHandler.sendToAllAround(new PC_PacketTileEntityMessageSTC(this, nbtTagCompound), this.worldObj,
					this.xCoord, this.yCoord, this.zCoord, 32);
		}
	}
